    ...
    public class Startup
    {
        ...
        private static IBuiltContainer CreateAppContainer(IServiceCollection services, IConfiguration configuration, ILoggerFactory loggerFactory)
        {
            ...
            return services.CreateAppBuilder(configuration, loggerFactory, typeResolver)
                .Bootstrap(c =>
                {
                    c.WithConfig<MessageDispatchConfig>(dispatch =>
                    {
                        dispatch.AddMessagePublisher<RedisPublisher>();
                    });
                    c.WithServices(reg =>
                     {
                         services.AddSingleton<ISerializationManager, SerializationManager>();
                         //services.AddHostedService<TestHostedService>();
                         services.AddSingleton<ITestSubscriptionService, TestSubscriptionService>();
                     });
                })
                .Build();
        }

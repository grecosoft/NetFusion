...
public static class HalResourceExtensions
{
	...
	public static HalResource<TModel>[] AsResources<TModel>(this IEnumerable<TModel> models)
		where TModel: class => models.Select(m => m.AsResource()).ToArray();
	...
	public static void EmbedResource<TModel>(this HalResource resource,
		...
		string named) where TModel: class => EmbedValue(resource, embeddedResource, named);
	...
	public static void EmbedModel<TModel>(this HalResource resource, TModel model, string named)
		where TModel: class => EmbedValue(resource, model, named);
	...
	public static void EmbedResources<TModel>(this HalResource resource, 
		...
		where TModel : class => EmbedValue(resource, embeddedResources, named);
	...
	public static void EmbedModels<TModel>(this HalResource resource, 
		IEnumerable<TModel> models, string named) where TModel: class => EmbedValue(resource, models, named);
	...
	private static void EmbedValue(HalResource resource, object value, string named)
	{
		...
		{
			throw new InvalidOperationException("The embedded name not specified.");
		}

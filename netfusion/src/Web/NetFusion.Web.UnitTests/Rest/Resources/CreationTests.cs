...
using NetFusion.Web.UnitTests.Rest.Resources.Models;
using Xunit;

namespace NetFusion.Web.UnitTests.Rest.Resources;
...
public class CreationTests
{
    ...
    public void EmbeddedName_Must_BeSpecified()
    {
        Assert.Throws<InvalidOperationException>(() =>
            ...
            .Message
            .Should()
            .Be("The embedded name not specified.");
    }
    ...
    {
        var entryModel = new EntryPointModel("10.00.1", "http://api.docs/finance/accounting.html");

        var resource = entryModel.AsResource();
        resource.Links = new Dictionary<string, Link>
        {
            { "payment", new Link("/api/finance/payments/{id}") },
            { "account", new Link("/api/finance/accounts/{id}") }
        };

...
namespace NetFusion.Common.Base.Properties;

/// <summary>
/// Class containing properties for an Url to be excluded from logs.  This can be 
...
public class UrlLogFilter
{
    ...
    internal UrlLogFilter(string url)
    {
        Url = url;
    }
}
...
public static class LogFilterExtensions
{
        ...
        string url, HttpStatusCode? statusCode = null)
    {
        ...
        var urlFilter = urlLogFilters.FirstOrDefault(f => f.Url == url) ?? new UrlLogFilter(url);

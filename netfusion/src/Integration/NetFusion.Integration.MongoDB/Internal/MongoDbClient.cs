...
namespace NetFusion.Integration.MongoDB.Internal;
    ...
    where TSettings : MongoSettings
{
    ...
    {
        if (DbSettings.ClientSettings == null && string.IsNullOrWhiteSpace(DbSettings.MongoUrl))
        {
            throw new InvalidOperationException(
                "Either the MongoDB URL or client settings must specified.");
        }
        
        if (DbSettings.ClientSettings != null)
        {
            return DbSettings.ClientSettings;
        }
    ...
    private void SetClientCredentials(MongoClientSettings clientSettings)
    {
        if (DbSettings.IsPasswordSet)
        {
            var credentials = MongoCredential.CreateCredential(
                ...
                throw new InvalidOperationException("AuthDatabaseName or DatabaseName not set"),
                DbSettings.UserName,
                DbSettings.Password);

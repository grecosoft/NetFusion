...
namespace NetFusion.RabbitMQ.Metadata
{
    ...
    public class ExchangeMeta
    {
        ...
        public static ExchangeMeta DefineDefault(string busName, string queueName,
            Action<QueueMeta> config = null)
        {
            if (string.IsNullOrWhiteSpace(busName)) throw new ArgumentException("Bus name not specified.", nameof(busName));
        ...
        {
            if (string.IsNullOrWhiteSpace(busName)) throw new ArgumentException("Bus name not specified.", nameof(busName));
            if (string.IsNullOrWhiteSpace(exchangeName)) throw new ArgumentException("Exchange name not specified.", nameof(exchangeName));
            if (string.IsNullOrWhiteSpace(exchangeType)) throw new ArgumentException("Exchange type not specified", nameof(exchangeType));

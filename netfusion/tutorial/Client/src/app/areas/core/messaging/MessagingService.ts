...
export class MessagingService {
    ...
    public dispatchQuery(): Observable<ApiResponse<any[]>> {
        let link = this.api.apiEntry.getLink('messaging-query');
        let request = ApiRequest.fromLink(link, config => config.withRouteValues({customerId: 10}));

        return this.api.client.send<any[]>(request);
    }
}